...
import classes from "./LiveGraph.module.css";
...
class CasesGraph extends Component {
  ...
  activateDetector = () => {
    ...
    axios
      .get(process.env.REACT_APP_SERVER + "/activateDetector")
      .then((response) => {
  ...
  getViolations = () => {
    axios
      .get(process.env.REACT_APP_SERVER + "/getViolation")
      .then((response) => {
    ...
    return (
      <div className="row">
        <div className="col-8 text-center">
          <h1 className={classes.heading}>Live Violations Graph</h1>
        ...
        <div className="col-4 mt-5">
          <div>
            ...
            <div onClick={this.activateDetector} className="btn btn-danger m-auto text-center">
              Activate Detector
            </div>
            <h6>
              {this.state.activatingDetector
                ? "Activating the detector. It may take a a minute or so.."
                : null}
            </h6>
            <h6>
              {this.state.liveDataOn
                ? "You are now seeing the live violation percentage."
                : null}
            </h6>
          </div>
        </div>
